...
def ingest_data(datestr, cycle, frame):
    ...
    open(f'/Users/clamalo/documents/harpnet/oper/data/gfs.f{frame:03d}.grib', 'wb').write(r.content)

if ingest:
    files = glob.glob('/Users/clamalo/documents/harpnet/oper/data/*')
    for f in files:
        os.remove(f)
    for frame in tqdm(frames):
        ingest_data(datestr, cycle, frame)

gfs_ds = xr.load_dataset('/Users/clamalo/documents/harpnet/oper/data/gfs.f003.grib', engine='cfgrib')
...
for domain in tqdm(available_domains):
    ...
    master_ds = xr.open_mfdataset(f'/Users/clamalo/documents/harpnet/oper/data/*.grib', engine='cfgrib', combine='nested', concat_dim='valid_time')
