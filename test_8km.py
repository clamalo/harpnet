...
if LOAD:
    ...
    with ProgressBar():
        ...
        ds = ds.coarsen(lat=2, lon=2, boundary='pad').mean()
    ...
    reference_ds = reference_ds.sel(latitude=slice(max_lat-1, min_lat+1), longitude=slice(min_lon+1, max_lon-1))

    input_ds = ds.interp(lat=reference_ds.latitude.values, lon=reference_ds.longitude.values)

    #crop both datasets to after october 1 2020 0z (not inclusive) THIS IS THE TRAIN/TEST SPLIT
    ...
    ds = ds.sel(lat=slice(max_lat-2, min_lat+2), lon=slice(min_lon+2, max_lon-2))
    ds = ds.isel(lat=slice(3, -2), lon=slice(3, -2))
...
model = UNetWithAttention(1, 1, (64,64)).to('mps').eval()
criterion = nn.MSELoss()
for epoch in range(6,11):
    if epoch == 5:
        continue
    original_checkpoint = torch.load(f'/Volumes/seagate/8km_checkpoints/train_e{epoch}.pt')
    model_weights = original_checkpoint['model_state_dict']
    train_loss = original_checkpoint['loss']
    ...
    with torch.no_grad():
        for i, (data, labels, days) in tqdm(enumerate(test_dataloader), total=len(test_dataloader)):
            ...
            # # plot input, output, and label
            # fig, ax = plt.subplots(1, 3, figsize=(10, 5))
            # ax[0].imshow(data[0, 0, :, :].cpu().numpy(), cmap='viridis', vmin=0, vmax=10)
            # ax[0].set_title('Input')
            # ax[1].imshow(output[0, 0, :, :].cpu().numpy(), cmap='viridis', vmin=0, vmax=10)
            # ax[1].set_title('Output')
            # ax[2].imshow(labels[0, 0, :, :].cpu().numpy(), cmap='viridis', vmin=0, vmax=10)
    ...
    torch.save(checkpoint, f'/Users/clamalo/documents/harpnet/8km_checkpoints/train_e{epoch}.pt')