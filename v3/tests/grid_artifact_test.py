...
# Load model checkpoint for a single tile (assuming such a checkpoint exists)
checkpoint_path = f"{tile}_model.pt"
if not constants.CHECKPOINTS_DIR.joinpath(f"{tile}_model.pt").exists():
    print(f"Checkpoint for tile {tile} not found. Please ensure it's available.")
else:
    model = UNetWithAttention().to(constants.TORCH_DEVICE)
    checkpoint = torch.load(checkpoint_path, map_location=constants.TORCH_DEVICE)
    ...
    model.eval()
    with torch.no_grad():
        for (inputs, elev_data, targets, times, tile_ids) in tqdm(test_dataloader, total=len(test_dataloader)):
            inputs, elev_data, targets = inputs.to(constants.TORCH_DEVICE), elev_data.to(constants.TORCH_DEVICE), targets.to(constants.TORCH_DEVICE)
