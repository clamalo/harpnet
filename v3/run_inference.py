"""
Executes inference over a specified date range, generating downscaled precipitation predictions.
Optionally loads a tile-specific best model if it exists, otherwise defaults to a global best model.

After performing inference, plots are saved to illustrate coarse inputs, model predictions,
and ground truth (depending on user preferences).
"""
...
from src.constants import (
    RAW_DIR, MODEL_NAME, TORCH_DEVICE,
    MIN_LAT, MAX_LAT, MIN_LON, MAX_LON, FIGURES_DIR, TILE_SIZE, FINE_RESOLUTION,
    CHECKPOINTS_DIR, NORMALIZATION_STATS_FILE, TILE_SIZE, PRE_MODEL_INTERPOLATION
)
import importlib
...
plot_coarse = False  # If True, plots coarse input fields
...
def load_model_for_tile(tile: int):
    """
    Loads either a tile-specific best model (if available) or the global best model.
    Returns a ready-to-eval model instance on the device.
    """
    device = TORCH_DEVICE
...
def get_tile_elevation(tile: int, elevation_ds):
    """
    Retrieves the elevation array for the specified tile, normalized by the highest elevation (8848.9).
    Returns a float32 array of shape [1, Hf, Wf].
    """
...
def prepare_inputs_for_tile(tile: int, day_ds, elevation_ds, model_device):
    """
    Given a day's data and a tile index, retrieves the coarse precipitation and interpolates
    it to TILE_SIZE x TILE_SIZE. Also retrieves and interpolates elevation data.
    Normalizes precipitation with mean_val/std_val, while elevation remains unchanged.
    Returns (times, inputs_tensor, coarse_for_plot).
    """
    coarse_latitudes, coarse_longitudes, fine_latitudes, fine_longitudes = tile_coordinates(tile)
    ...
    # --- UPSAMPLE TO TILE_SIZE x TILE_SIZE ---
    coarse_tp_64 = torch.nn.functional.interpolate(
        coarse_tp_torch, size=(TILE_SIZE, TILE_SIZE), mode=PRE_MODEL_INTERPOLATION
    )
    elev_64 = torch.nn.functional.interpolate(
        elev_fine_torch.unsqueeze(0).expand(T, -1, -1, -1),
        size=(TILE_SIZE, TILE_SIZE),
        mode=PRE_MODEL_INTERPOLATION
    )
    elev_64 = elev_64.squeeze(0)

    # --- NORMALIZE PRECIP, COMBINE WITH ELEVATION ---
    coarse_tp_64_norm = (coarse_tp_64 - mean_val) / std_val
    inputs = torch.cat([coarse_tp_64_norm, elev_64], dim=1)  # shape (T,2,H,W)

    coarse_tp_64_mm = coarse_tp_64.squeeze(1).cpu().numpy()  # For potential plotting
    return times, inputs, coarse_tp_64_mm

def get_tile_ground_truth(tile: int, day_ds):
    """
    Returns the day_ds data for the specified tile at fine resolution,
    without normalization or additional transformations.
    """
...
def stitch_tiles(tile_data_dict, tile_list):
    """
    Merges tile-level data into a single 2D array covering the entire domain.
    This is used to create a seamless mosaic of the model predictions, coarse input, or truth.
    """
    ...
    # --- DETERMINE TIME STEPS FROM FIRST TILE ---
    T = None
    for t in tile_list:
...
def plot_and_save_images(stitched_coarse, stitched_model, stitched_truth, times, year, month, day, plot_coarse):
    """
    For each time step, plots coarse input (optional), model output, and ground truth
    as separate subplots in a single figure. Saves each time step as a .png in OUTPUT_DIR.
    """
    vmax_val = 0.5
    cmap_choice = 'viridis'
    ...
    for i, t in enumerate(times):
        if plot_coarse:
            ...
            im1 = ax1.imshow(
                stitched_coarse[i], origin='lower', cmap=cmap_choice,
            ...
            im2 = ax2.imshow(
                stitched_model[i], origin='lower', cmap=cmap_choice,
                extent=extent, transform=ccrs.PlateCarree(),
                vmin=0, vmax=vmax_val
            )
            ...
            im3 = ax3.imshow(
                stitched_truth[i], origin='lower', cmap=cmap_choice,
                extent=extent, transform=ccrs.PlateCarree(),
                vmin=0, vmax=vmax_val
            )
...
def plot_total_precipitation(total_coarse, total_model, total_truth, plot_coarse):
    """
    After all daily inferences are complete, plots the total precipitation over the entire period.
    If plot_coarse is True, includes coarse total; otherwise only model vs. truth.
    """
    ...
    if plot_coarse:
        ...
        im1 = ax1.imshow(
            total_coarse, origin='lower', cmap=cmap_choice,
            extent=extent, transform=ccrs.PlateCarree(),
            vmin=0, vmax=vmax_val
        )
        ...
        im2 = ax2.imshow(
            total_model, origin='lower', cmap=cmap_choice,
            extent=extent, transform=ccrs.PlateCarree(),
            vmin=0, vmax=vmax_val
        )
        ...
        im3 = ax3.imshow(
            total_truth, origin='lower', cmap=cmap_choice,
            extent=extent, transform=ccrs.PlateCarree(),
            vmin=0, vmax=vmax_val
        )
...
def process_day(elevation_ds, day_slice, year, month, day):
    """
    Processes a single day by running inference across all tiles, then stitches 
    tile-level predictions/ground truth/coarse data to form domain-wide arrays.
    Updates the total precipitation arrays, and calls plot_and_save_images for each time step.
    """
    ...
    for tile in valid_tiles:
        ...
        with torch.no_grad():
            preds_norm = model(inputs)  # normalized predictions
        ...
        fine_tp_torch = torch.from_numpy(fine_tp).unsqueeze(1)
        fine_tp_64 = torch.nn.functional.interpolate(
            fine_tp_torch, size=(TILE_SIZE, TILE_SIZE), mode=PRE_MODEL_INTERPOLATION
        ).squeeze(1).numpy()
    ...
    plot_and_save_images(
        stitched_coarse_inches, stitched_model_inches, stitched_truth_inches,
...
def main():
    """
    Main inference pipeline. Iterates through each day in the specified date range, 
    loading daily NetCDF data. If the file is missing, logs a warning and moves on.
    After processing all days, plots the total precipitation from model vs. truth (and coarse if chosen).
    """
