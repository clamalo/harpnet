...
def predict(epoch, gfs_arr):
    model = UNetWithAttention(1, 1, (64,64)).to('mps')
    model.load_state_dict(torch.load(f'/Users/clamalo/documents/harpnet/8km_checkpoints/train_e{epoch}.pt')['model_state_dict'])
    model.eval()
    with torch.no_grad():
        batch_size = 64
        gfs_pred = []
        for i in tqdm(range(0, len(gfs_arr), batch_size)):
            gfs_arr_batch = gfs_arr[i:i+batch_size].to('mps')
            gfs_pred_batch = model(gfs_arr_batch).squeeze()
            gfs_pred.append(gfs_pred_batch)
            torch.cuda.empty_cache()
        gfs_pred = torch.cat(gfs_pred)
    return gfs_pred


members_to_process = [45, 20, 44, 36, 28]
...
ax.set_title('8km Downscaled Output / 0.25 Deg Input')
...
for location, coordinates in stations.items():
    ...
    ax.set_title(f'{location} Output - 8km')
    # ax.legend()
